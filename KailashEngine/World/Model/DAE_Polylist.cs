...
namespace KailashEngine.World.Model
{
    class DAE_Polylist : Mesh
    {
        ...
        public void load(
            ...
            List<Vector3> mesh_positions, List<Vector2> mesh_uvs, List<Vector3> mesh_normals)
        {
        ...
        public void load(
            Grendgine_Collada_Polylist polylist,
            Dictionary<String, DAE_Material> material_collection,
            List<Vector3> mesh_positions, List<Vector2> mesh_uvs, List<Vector3> mesh_normals, List<Vector4> mesh_bone_ids, List<Vector4> mesh_bone_weights)
        {
            // Exit if faces are not triangulated
            if (!polylist.VCount.Value_As_String.Contains("3"))
            {
